...
// Handle export request (CHANGED: POST instead of GET, token in body)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['download'])) {

    // Verify CSRF token (accept both names for compatibility)
    $postedToken = $_POST['csrf_token'] ?? ($_POST['token'] ?? null);
    if (!$postedToken || !isset($_SESSION['csrf_token']) || !hash_equals($_SESSION['csrf_token'], $postedToken)) {
        die("CSRF token validation failed.");
    }

    $exportType = $_POST['download'];

    // Same behavior as original: ids is a comma-separated string
    $selectedIds = isset($_POST['ids']) ? explode(',', $_POST['ids']) : array_keys($zettels);
...
function convertInternalLinks($content, $allZettels, $format = 'markdown') {
    ...
    return preg_replace_callback('/\\[\\[([a-zA-Z0-9]+)\\]\\]/', function($matches) use ($allZettels, $format) {
        ...
        if (isset($allZettels[$id])) {
            ...
            if ($format === 'markdown') {
                // Original behavior: link to anchors within the single-file export
                return '[' . $title . '](#' . $id . ')';
            }
        }
        return '[[' . $id . ']]';
...
function exportAsSingleMarkdown($exportZettels, $allZettels) {
    ...
    foreach ($exportZettels as $id => $zettel) {
        ...
        // IMPORTANT (original feature): anchor so (#ID) works reliably in markdown viewers
...
<head>
  ...
  <style>
    .export-container { max-width: 900px; margin: 0 auto; padding: 20px; }
    .export-options { background: white; padding: 30px; border-radius: 8px; margin-bottom: 30px; box-shadow: 0 2px 5px rgba(0,0,0,0.05); }
    .export-format { margin: 20px 0; padding: 20px; border: 2px solid #ecf0f1; border-radius: 8px; transition: border-color 0.3s; }
    .export-format:hover { border-color: #3498db; }
    .export-format h3 { margin-top: 0; color: #2c3e50; }
    .export-format p { color: #7f8c8d; margin: 10px 0; }
    .zettel-list { background: white; padding: 20px; border-radius: 8px; box-shadow: 0 2px 5px rgba(0,0,0,0.05); }
    .zettel-checkbox { display: flex; align-items: center; padding: 10px; margin: 5px 0; background: #f8f9fa; border-radius: 4px; transition: background 0.2s; }
    .zettel-checkbox:hover { background: #e9ecef; }
    .zettel-checkbox input[type="checkbox"] { margin-right: 10px; width: 18px; height: 18px; cursor: pointer; }
    .zettel-checkbox label { cursor: pointer; flex: 1; font-weight: 500; }
    .zettel-checkbox .zettel-id { color: #7f8c8d; font-size: 0.85em; margin-left: 10px; }
    .selection-controls { margin: 20px 0; padding: 15px; background: #f8f9fa; border-radius: 4px; display: flex; gap: 10px; flex-wrap: wrap; }
    .btn-secondary { background: #95a5a6; color: white; padding: 8px 16px; border: none; border-radius: 4px; cursor: pointer; font-size: 0.9em; font-weight: 600; }
    .btn-secondary:hover { background: #7f8c8d; }
    .export-actions { margin-top: 30px; display: flex; gap: 15px; flex-wrap: wrap; }
    .btn-export { background: #27ae60; color: white; padding: 12px 24px; border: none; border-radius: 4px; cursor: pointer; font-size: 1em; font-weight: 600; text-decoration: none; display: inline-block; transition: background 0.3s; }
    .btn-export:hover { background: #229954; }
    .btn-export.disabled { background: #bdc3c7; cursor: not-allowed; }
    .stats { margin: 15px 0; padding: 10px; background: #e8f5e9; border-radius: 4px; color: #2e7d32; font-weight: 600; }
  </style>
</head>
<body>
  <div class="export-container">
    ...
    <div class="zettel-list">
      ...
      <div class="stats" id="selectionStats">
        <span id="selectedCount"><?php echo count($zettels); ?></span> of <?php echo count($zettels); ?> notes selected
      </div>

      <!-- CHANGED: POST instead of GET; token field renamed to csrf_token -->
      <form id="exportForm" method="POST">
        ...
        <?php foreach ($zettels as $id => $zettel): ?>
          <div class="zettel-checkbox">
            <input
              type="checkbox"
              id="zettel-<?php echo htmlspecialchars($id); ?>"
              value="<?php echo htmlspecialchars($id); ?>"
              class="zettel-select"
              checked
              onchange="updateSelectionCount()"
            >
            <label for="zettel-<?php echo htmlspecialchars($id); ?>">
              <?php echo htmlspecialchars($zettel['title']); ?>
              <span class="zettel-id"><?php echo htmlspecialchars($id); ?></span>
            </label>
          </div>
        <?php endforeach; ?>
      </form>
      ...
      <div class="export-actions">
        <button class="btn-export" onclick="exportAs('single')">Export as Single Markdown</button>
        <button class="btn-export" onclick="exportAs('zip')">Export as ZIP</button>
        <button class="btn-export" onclick="exportAs('json')">Export as JSON</button>
      </div>
    </div>
  </div>
  ...
  <script>
    ...
    function exportAs(type) {
      const selected = Array.from(document.querySelectorAll('.zettel-select:checked')).map(cb => cb.value);
